...
// Lê os dados realizando a separação por quebra de linha, ie, \n.
var parser = new Readline();
port.pipe(parser);
/**
 * Armazena cada amostra obtida no Buffer. Cada elemento do Buffer possui
 * um instânte de tempo associado a ele e representa quando a amostragem
 * foi realizada.
 */
var dataBuffer = [];
parser.on('data', function (valueStr) {
    var valueFloat = parseFloat(valueStr);
    dataBuffer.push({ x: new Date(), y: valueFloat });
});
/**
